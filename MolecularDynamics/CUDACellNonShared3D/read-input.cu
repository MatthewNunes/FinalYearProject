...
int read_input (float *sigma, float *rcut, float *dt, float *eqtemp, float *dens, int *nstep, int *nequil, int *iscale, int *iprint, int *nc, int *tbx, int *tby, int *tbz)
{
   int ierr=1, natoms, m;
...
//   *nc = 16;
   ...
   printf ("\n ENTER THE THREAD BLOCK DIMENSIONS");
   ierr &= scanf("%d %d %d",tbx, tby, tbz)/3;
