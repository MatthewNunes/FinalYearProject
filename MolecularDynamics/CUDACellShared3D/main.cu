...
//   CUDA_CHECK_RETURN(cudaMemcpy(d_virial, vArray, sizeof(float) * (mx+2) * (my+2) * (mz+2)  , cudaMemcpyHostToDevice));
   ...
   int  maxPart = maxParticles(head, list, gtx, gty, gtz);
   int sharedSize = (2 + maxPart*3) * sizeof(float) * tbx*tby*tbz;
//   printf(" SHARED SIZE  %d BYTES\n",sharedSize);
//   printf("I get to the kernel\n");
//   fflush(stdout);
   startTime = get_tick();
   force<<<dimGrid, dimBlock, sharedSize>>>(maxPart, d_potential, d_virial, d_rx, d_ry, d_rz, d_fx, d_fy, d_fz, sigma, rcut, vrcut, dvrc12, dvrcut, d_head, d_list, mx, my, mz, gtx, gty, gtz);
...
//      CUDA_CHECK_RETURN(cudaMemcpy(d_potential, pArray, sizeof(float) * (mx+2) * (my+2) * (mz+2)  , cudaMemcpyHostToDevice));
//      CUDA_CHECK_RETURN(cudaMemcpy(d_virial, vArray, sizeof(float) * (mx+2) * (my+2) * (mz+2)  , cudaMemcpyHostToDevice));
      maxPart = maxParticles(head, list, gtx, gty, gtz);
      sharedSize = (2+maxPart * 3) * sizeof(float) * tbx*tby*tbz;
